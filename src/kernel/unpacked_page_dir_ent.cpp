...
unpacked_page_dir_ent::unpacked_page_dir_ent(unsigned int * addr, unsigned char extra, bool global, bool pat, bool size, bool accessed,
                                                 bool cache, bool write_through, bool priv, bool read_write, bool present)
{
    physical_page_address_ =  addr; //(int *)((int)addr | (0<<11)); //zero out bits 0-11
    extra_ = extra;
    global_ = global;
    pat_ = pat;
    size_ = size;
...
unpacked_page_dir_ent::unpacked_page_dir_ent(unsigned int int_form )
{
    ...
    size_ = (bool)(int_form | 0x00000080);
    pat_ = (bool)(int_form | 0x00000040);
...
unsigned int unpacked_page_dir_ent::return_as_dir_entry()
{
    int entry = ((unsigned int)physical_page_address_ | (((int)extra_) << 9) | (((int)global_) << 8) | (((int)pat_) << 7) | (((int)size_) << 6) | (((int)accessed_) << 5) | 
